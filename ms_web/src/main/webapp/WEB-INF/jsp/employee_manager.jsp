...
<table class="table table-hover table-bordered">
    ...
    <c:forEach items="${pageInfo.list}" var="employee">
        <tr style="background-color: rgba(242,242,242, .4);">
            ...
            <th id="realname">${employee.realName}</th>
            <th id="sexid"><c:if test="${employee.sexId=='0'}">男</c:if> <c:if
                    test="${employee.sexId=='1'}">女</c:if></th>
            <th id="departmentname">${employee.department.departmentName}</th>
            <th id="rolename">${employee.role.roleName}</th>
            <th id="telephone">${employee.telephone}</th>
            <th><button class='btn btn-primary edit_button' type='submit'>编&nbsp;辑</button>&nbsp;&nbsp;&nbsp;&nbsp;
                <button class='btn btn-primary delete_button' type='submit'>停&nbsp;用</button></th>
        </tr>
    </c:forEach>
    </thead>
</table>
